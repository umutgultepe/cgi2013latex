...
\usepackage{tabularx}
\usepackage{epstopdf}
\journalname{The Visual Computer} % The correct name will be entered by the editor
\begin{document}

\title{Real-time Virtual Fitting Room with \\Body Measurement and Motion Smoothing}

\author{Umut G\"{u}ltepe  \and U\u{g}ur G\"{u}d\"{u}kbay}
\institute{Umut G\"{u}ltepe  \and U\u{g}ur G\"{u}d\"{u}kbay \at 
Department of Computer Engineering, Bilkent University, Ankara, Turkey \\
Tel.: +90-312-2901386; Fax: +90-312-2664047 \\
              \email{\{gultepe,gudukbay\}@cs.bilkent.edu.tr}}
\date{ }% The correct dates will be entered by the editor

\maketitle

\begin{abstract} 
We present a novel virtual fitting room framework using a depth sensor, which provides a realistic fitting experience with customized motion filters, size adjustments and physical simulation. The proposed scaling method adjusts the avatar and determines a specific apparel size, prepares the physics simulation and collision detection environment, with a total preprocessing time of one second, eliminating the requirement for long preprocessing times to achieve a realistic fitting experience. The real-time motion filters prevent unnatural artifacts due to the noise from depth sensor or self-occluded body parts. 
...
We present a novel virtual fitting room framework that provides all the basic features expected from such an application, along with enhancements in various aspects for higher realism. These enhancements include motion filtering, customized user scaling, and physics engine.

Motion filtering process starts with temporal averaging of joint position, in order to overcome the high noise of the depth sensor. However, temporal averaging does not prove to be sufficient, as unnatural movements take place due to limited recognition capabilities and self-occlusion. Therefore we implemented customized joint angle filters, along with bone splitting to let limbs twist in a more natural way.

The cloth pieces to be fitted on the user's avatar must first be scaled accordingly. It is for this reason we implemented the body measurement process, which starts with depth map smoothing, in order to reduce the noise. Afterwards, we utilize the filtered depth map along with filtered user joints to measure a set of parameters, which are used in conjunction to estimate body height and width. These parameters are averaged over time in the end, to minimize the error.

We provide information about the algorithms and implementation details, with visual examples and statistical data. Before the conclusion, we compare our results with other state of the art approaches to demonstrate the performance of our system.  

Physics engine utilizes collision spheres and capsules to perform collision detection. Therefore the correct sphere radii and positions are also determined during body measurements. The virtual avatar is aligned with a set of invisible spheres and capsules aligned with joints and limbs respectively which are updated in real time and used in collision detection. Cloth particles are also affected by gravity and inertia.
...
Section~\ref{sec:Overall} provides an overview of the proposed approach, as well as the details of the cloth simulation engine, depth map filtering, body measurements, temporal averaging, and techniques used to cope with the inferior motion data problems.
...
\begin{figure*}
	\centerline{
	\fbox{\includegraphics[width=1.98\columnwidth]{./figures/rotation-filter.eps}}
	}
\centerline{\ }
\caption{The row and filtered samples for the right humerus roll angle}
...
The proposed depth map enhancement algorithm is implemented on the in-house developed virtual dressing framework, which acts as the test bench for experimentation. The measurement process to identify the body parameters starts after a subject is recognized, identified and calibrated for tracking. Following the parameter estimation, the virtual avatar and cloth are created and the simulation starts.

Table~\ref{tbl:body_results} presents the measurements of the body dimensions, as well as the errors and the standard deviations of the measurements in 30 frames for three subjects. Our results are compared with results from~\cite{Giovanni2012} and \cite{Samejima2012}, which are also real-time applications, in Table~\ref{tbl:performance_compare}.
...
\end{table} 
The total time for the measurement algorithm do not exceed 1.005 seconds in any of the cases. Considering 1.0 seconds is the time for acquiring 30 frames of input from the depth sensor, it is safe to say that the proposed algorithm does not introduce any delays and problems for a real time application because it needs to run only once at the beginning of the simulation.
...
\begin{figure*} 
	...
	\centerline{\ }
	\centerline{
	\includegraphics[width=0.50\columnwidth]{./figures/flightsuit-1.eps}
	\includegraphics[width=0.50\columnwidth]{./figures/flightsuit-2.eps}
	\includegraphics[width=0.50\columnwidth]{./figures/flightsuit-3.eps}
	\includegraphics[width=0.50\columnwidth]{./figures/flightsuit-4.eps}
	}
	\centerline{(b)}
	\centerline{\ }
	\centerline{
	\includegraphics[width=0.50\columnwidth]{./figures/jeans-1.eps}
	\includegraphics[width=0.50\columnwidth]{./figures/jeans-2.eps}
	\includegraphics[width=0.50\columnwidth]{./figures/jeans-3.eps}
	\includegraphics[width=0.50\columnwidth]{./figures/jeans-4.eps}
	}
	\centerline{(c)}
	\caption{Examples of different dresses on a model with different postures: (a) sun dress, (b) flight suit and (c) jeans and vest.}
	\label{fig:examples}
\end{figure*}
...
\bibitem{Azimi2012}
Azimi M.: Skeletal Joint Smoothing, White Paper, Microsoft Corp. Available at \verb+http://msdn.microsoft.com/en-us/library/jj131429.aspx+ [Online] (2012)
...
S. Raiten.: Kinect – Getting Started – Become The Incredible Hulk. Available at \verb+http://www.codeproject.com/Articles/213034/Kinect-+ \verb+Getting-Started-Become-The-Incredible-Hulk+ (2011).
...
Giovanni, S., Choi, Y.C., Huang J., Tat K.E. and Yin, K.: Virtual Try-On Using Kinect and HD Camera. Proceedings of the 
5th International Conference on Motion in Games (MIG'12). Lecture Notes in Computer Science, vol. 7660, page 55-65. 
Springer, (2012)
...
Hauswiesner, S., Straka, M., and Reitmayr, G.: Virtual try-on through image-based rendering. IEEE Transactions on Visualization
...
\bibitem{Holt1957}
Holt, C.C.:  Forecasting trends and seasonals by exponentially weighted averages. Carnegie Institute of Technology, Pittsburgh ONR memorandum, No. 52 (1957)

\bibitem{Kalekar2004}
Kalekar, P. S.: Time series forecasting using Holt-Winters exponential smoothing. Kanwal Rekhi School of Information Technology (2004).
...
\bibitem{Kim2013}
Kim, D.-E., and LaBat, K.: Consumer experience in using 3D virtual garment simulation technology. 
Journal of The Textile Institute. In press
...
\bibitem{Meng2010}
Meng, Y., Mok, P. Y. and Jin, X.: Interactive virtual try-on clothing design systems. Computer-Aided Design, 42(4), 310-321, (2010)

\bibitem{Mullera2007}
M\"{u}ller M., Heidelberger B, Hennix M., Ratcliff J.: Position based dynamics. Journal of Visual Communication and Image Representation, 18(2), 109-118 (2007)
...
Samejima, I., Maki, K., Kagami, S., Kouchi, M., and Mizoguchi, H.: A body dimensions estimation method of subject from a few measurement items using KINECT. Proceedings of the IEEE International Conference on Systems, Man, and Cybernetics (SMC) (2012)

\bibitem{Styku2013}
Styku, Inc.: Virtual Fitting Room and Body Scanning. Available at \verb+http://www.styku.com/+ (2013).

\bibitem{Tonge2010}
Tonge, R.: Collision detection in PhysX, Recent Advances in Real-Time Collision and Proximity Computations for
...
\bibitem {Yasseen2013}
Yasseen, Z., Nasri, A., Boukaram, W., Volino, P., and Magnenat-Thalmann, N.: Sketch-based garment design with quad meshes. Computer-Aided Design, 45(2), 562-567 (2013)
...
\bibitem{Zhou2012}
Zhou, Z., Shu, B., Zhuo, S., Deng, X., Tan, P., and Lin, S.: Image-based clothes animation for virtual fitting. In SIGGRAPH Asia Technical Briefs. Article 33, 4 pages, ACM, New York, NY, USA (2012)
