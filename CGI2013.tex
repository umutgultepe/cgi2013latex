...
We present a novel virtual fitting room framework which provides all the basic feautures expected from such an application, along with enhancements in various aspects for higher realism. These enhancements include motion filtering, customized user scaling and physics engine.
...
Section~\ref{sec:Physics} explaines the cloth simulation engine and purposes of the user measurements. 
...
\label{sec:Overall}
The main objective of a virtual dressing room is giving the user the ideo of how the apparel will look on him/her without actually trying it on. As in any simulation, the absolute reality is almost impossible to attain, although with garment simulation and depth sensing technologies, a substantial level of realism can be achieved.
...
As our only problematic bones for this particular case is the upper limbs, we employed a novel approach to solve this problem by introducing an additional bone connected in series for the upper limbs. The Humerus and Ulna bones are split halfway and the lower sections are labeled as Humerus-Extention and Ulna-Extention respectively. The vertex weights in the corresponding sections are divided linearly among two sections as seen in Figure \ref{fig:forearm-weights}. 
...
The optimization algorithm described in Section~~\ref{sec:3} is implemented on the in-house developed virtual dressing framework, which acts as the testbench for experimentation. 
...
Table~\ref{tbl:body_results} presents the measurements of the body dimensions, as well as the errors and the standart deviations of the measurements in 30 frames for three subjects. Our results are compared with results from \cite{Giovanni2012}, which is also a real-time applicaiton, in Table \ref{tbl:performance_compare}.
...
\hline
  ...
  Error Average  & 1.2 \% & 4 \% \\ \hline
  Error Deviation & 0.5 \%  & 3 \% \\ \hline
\end{tabular}
\end{center}
...
\bibitem {HANIM}
H-Anim Web Site. www.h-anim.org
