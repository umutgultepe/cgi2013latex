...
We present a new approach to adjusting a virtual cloth and an avatar for a specific subject on the fly. Constraining the overall preprocessing time to one second, the clothing piece and the avatar are adjusted automatically and prepared for a physics powered virtual fitting room framework. 
...
Section~\ref{sec:3} describes the proposed methodology, including the details of depth map optimization, body measurements, and temporal optimization. Section~\ref{sec:4} provides experimental results. Section~\ref{sec:5} gives conclusions and further research directions. 
...
\item 
The depth map represents the measurement will instead perform measurements based on the pixel distribution in the optimized subject depth map. 
\end{itemize}
...
\section{Experiments}
\label{sec:4}
The optimization algorithm described in Section~~\ref{sec:4} is implemented on a virtual dressing framework, which provides a set of boiler plate features. The framework provides the testbench for experimentation, providing the following basic functions:

\begin{itemize}
  \item 3D scene rendering and render cycle management, 
  \item cloth simulation framework, and
  \item depth sensor integration.
\end{itemize} 

The optimization process runs after a subject is recognized, identified and calibrated for tracking. Following the parameter estimation, the virtual avatar and cloth are created and the simulation starts.
...
The total time for the optimization algorithm do not exceed 1.005 seconds in any of the cases. Considering 1.0 seconds is the time for acquiring 30 frames of input from the depth sensor, it is safe to say that the proposed algorithm does not introduce any delays and problems for a real time application becuase it needs to run only once in the beginning of the simulation.
